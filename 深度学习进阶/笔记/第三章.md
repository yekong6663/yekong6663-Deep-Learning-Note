...
def evaluate_accuracy(net, data_iter):
    ...
    返回:
        精度（正确率）
    """
    # 如果是 PyTorch 的 nn.Module，切换到评估模式（关闭 dropout 等）
    if isinstance(net, torch.nn.Module):
        net.eval()
    
    # Accumulator(2) 用于累加：正确预测数、总预测数
...
def train_epoch_ch3(net, train_iter, loss, updater):
    ...
    参数:
        ...
        train_iter: 训练数据迭代器
        loss: 损失函数
        updater: 参数更新函数（可以是自定义的 sgd 或 PyTorch 优化器）
    ...
    # Accumulator(3): 训练损失总和、训练准确度总和、样本数
    ...
    for X, y in train_iter:
        ...
        if isinstance(updater, torch.optim.Optimizer):
            ...
            l.mean().backward()       # 反向传播（MSELoss 返回平均损失）
            updater.step()            # 更新参数
        else:
            # 使用自定义优化器
            l.sum().backward()        # 反向传播
            updater(X.shape[0])       # 更新参数（传入 batch_size）
...
def predict_ch3(net, test_iter, n=6):
    ...
    # 预测标签（文本形式）
    # net(X).argmax(axis=1) 获取每行最大概率的索引（预测类别）
