...
>应注意，交叉熵权误差的本质是***取出预测正确的概率***，这样就可以比较好的理解one-hot与not one-hot两种形式
#### python实现
```python
...
>最后的计算除以batch_size，其实际上就是y，t的行数（一般都是用一行表示一个变量），使用shape属性的第一个元素。针对y是一维数组的情况（列向量），实际上是进行了一次转置操作。
#### 非one-hot数据
```python
def CrossEntropyError(y,t):
    if y.ndim == 1:#把向量变为矩阵
...
>eg.batch_size=5时，np.arange(batch_size)会生成array([0, 1, 2, 3, 4])，若t为array([2, 7, 0, 9, 4])，则二者一组合会生成：
array(\[ [y[0,2], y[1,7], y[2,0], y[3,9], y[4,4] ]).
...
>首先定义极小量 $h$ 和`grad`的`numpy`形状，此处使用`np.zeros_like(x)`可以生成一个形状与`numpy`数组一样的但是值全为0的numpy数组。
>遍历，使用for循环遍历，每一次对x[idx]元进行计算，要实现使用`temp`来存储。
>计算 $f(x+h)$和$f(x-h)$ ，注意此处的x实质上应该是只有`x[idx]`变化的（$±h$）的`numpy`数组，所以每一次需要传入整个`numpy`数组，但是需要单独修改`x[idx]`。
>将计算得出的偏导存储入`grad numpy`数组。
### 4.4.3 梯度的含义
梯度会指向各点处的函数值降低的方向。
...
>因为数据是随机选择的`mini-batch`数据，所以又称为随机梯度下降法（stochastic gradient descent）。
